...
class MCTS:
    ...
    #     root = MCTSNode(None)
    #     self.samples = 0
    #     while self.samples < self.sample_size: # do 100 just to check
    ...
    def runMCTS(self, state, turn_count):
        root = MCTSNode(None)
        self.samples = 0
        ...
        while self.samples < self.sample_size: # Adjust number as needed.
            value = self.MCTS(root, state)
        ...
        adjusted_visits = visit_counts ** (1/self.tau) # Adjusting the visits using tau
        adjusted_probs = adjusted_visits / np.sum(adjusted_visits) # Normalize to get probabilities

        for ind, child in enumerate(root.children):
            stats.append(Node(child.move.getFrom(), child.move.getTo(), child.visit_count, child.value, child.p))
            # Now choose the move based on the adjusted probabilities
            prob = adjusted_probs[ind]
            if prob > max_num_moves:
                max_num_moves = prob
                candidates = [child.move]
            elif prob == max_num_moves:
                candidates.append(child.move)
