...
class ExperienceData:
    def __init__(self, board: NDArray[np.float32], value: float, policy: NDArray[np.float32], mask: NDArray[np.float32]) -> None:
        self.board: NDArray[np.float32] = board
...
class ExperienceBuffer:
    ...
    def get_dataset(self, batch_size: int) -> Dataset:
        ...
        new_dataset.set(
            np.stack([d.board for d in self.data]), # (board_size, board_size) -> (N, board_size, board_size)
