...
def get_generated_gamedata_dataset(n: int | None = None, simulated: bool = False, sim_value_acc: float = 0.5, include_trivial: bool = True) -> tuple[Dataset, set[Board], set[Board]]:
    '''
    ...
    unique_boards_set_nt: set[Board] = set()
    total_value_acc = 0
    total_policy_acc = 0
    total_nt_policy_acc = 0
    ...
    for _, game_data_list in tqdm(game_data_lists):
        for game_data in game_data_list:
            ...
            for turn in turn_data:
                ...
                if not is_trivial:
                    unique_boards_set_nt.add(board)
                ...
                if experienced_outcome == ground_truth_outcome:
                    total_value_acc += 1
                policy_acc = policy_accuracy_function(experienced_policy, np.array(ground_truth_policy))
                if policy_acc:
                    total_policy_acc += 1
                if not is_trivial and policy_acc:
                    total_nt_policy_acc += 1
                ...
                if not is_trivial or include_trivial:
                    if simulated:
    ...
    logger.info(f"Found {num_unique_boards} unique boards in the game data (include_trivial={include_trivial}), out of {total_boards} total boards ({(num_unique_boards/total_boards)*100:.2f}%).")
    ...
    logger.info(f"Training data value accuracy: {total_value_acc/total_boards:.2%}, policy accuracy: {total_policy_acc/total_boards:.2%}, non-trivial policy accuracy: {total_nt_policy_acc/len(unique_boards_set_nt):.2%}.")
    ...
    return ev_dataset, unique_boards_set, unique_boards_set_nt
