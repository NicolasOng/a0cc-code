...
from a0_new.protocols.model import TrainableModel, T_nnx
from a0_new.dataset import Dataset
from a0_new.eval.dataset import evaluate_model
...
def loss_fn(
        ...
        batch: dict[str, Any]
    ) -> tuple[jnp.ndarray, tuple[jnp.ndarray, jnp.ndarray, float, float]]:
...
def train_model_epoch(
        ...
        save_fn: Optional[Callable[[T_nnx, int], None]] = None,
        cur_model_no: int = 0,
    ) -> tuple[T_nnx, EpochData, int]:
    ...
    assert num_batches > 0, "Dataset must contain at least one batch."
    ...
    for ts, batch in enumerate(batches):
        ...
        state_batch, value_batch, policy_batch, mask_batch = batch
        batch = {
            'board': state_batch,  # (N, board_size, board_size, 2)
...
def train_model_epochs(
        ...
        num_epochs: int = 1,
        test_datasets: dict[str, Dataset] = {},
        save_fn: Optional[Callable[[TrainableModel, int], None]] = None,
        save_type: str = "epoch",
        plot: bool = True
    ) -> tuple[TrainableModel, DatasetData]:
    """
    Train the model for a number of epochs on the given dataset.
    """
    save_batch_fn = save_fn if save_type == "batch" else None
    save_epoch_fn = save_fn if save_type == "epoch" else None
    ...
    for epoch in range(num_epochs):
        ...
        if save_epoch_fn is not None:
            # Save the model after each epoch
            logger.info(f"Saving model after epoch {epoch + 1}...")
            save_epoch_fn(model, epoch + 1)
